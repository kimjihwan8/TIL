...
어떻게 가능하냐면, `.yarn/cache`에 있는 zip 파일들을 git에 같이 올려놓기 때문이야.

---

# 왜 원래는 node_modules를 git에 안 올렸냐

node_modules는 git에 올리면 안 된다고 배웠을 거야. 이유가 있어.

**파일이 너무 많아**

react 하나만 설치해도 `node_modules` 안에 파일이 수천 개야. 패키지 100개면 수십만 개. git은 파일 하나하나를 추적하니까 커밋 속도가 엄청 느려지고 저장소 용량이 폭발해.

**OS마다 다를 수 있어**

일부 패키지는 설치할 때 OS에 맞는 바이너리를 컴파일해. 맥에서 올린 `node_modules`를 리눅스 CI에서 쓰면 동작 안 할 수 있어.
...
`node_modules`는 파일이 수십만 개인데, `.yarn/cache`는 패키지당 zip 파일 하나야. 패키지 100개면 zip 100개. git이 추적하기에 전혀 부담 없어.

**OS 독립적이야**

zip은 그냥 패키지 파일을 압축한 거라 OS랑 상관없어. 맥에서 올린 zip을 리눅스 CI에서 그대로 써도 돼.

**내용이 결정적이야**

같은 버전이면 항상 같은 zip이 나와. 어디서 받아도 동일하니까 git에 올려도 안전해.
